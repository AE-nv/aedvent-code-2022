...
impl CrateStack {
    ...
    pub fn add_n_crates(&mut self, crates_to_add: Vec<String>) {
        for crate_to_add in crates_to_add {
            self.add_crate(&crate_to_add);
        }
    }
    ...
    pub fn remove_top_n_crates(&mut self, amount: i32) -> Vec<String> {
        let mut crates = Vec::new();
        for _ in 0..amount {
            crates.push(self.remove_top_crate());
        }
        crates.reverse();
        crates
    }
